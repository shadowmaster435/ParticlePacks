...
public class Vector3 extends Vec3d {
    ...
    public Vector3 with_random_offset(float max_ofs) {
        ...
        var randx = MathHelper.nextBetween(Random.createLocal(), 0f, 1f);
        var randy = MathHelper.nextBetween(Random.createLocal(), 0f, 1f);
        var randz = MathHelper.nextBetween(Random.createLocal(), 0f, 1f);

        return new Vector3(this.add(min.lerp(max, new Vector3(randx, randy, randz))));
    }
